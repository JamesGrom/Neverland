...
class RegistrationViewController: UIViewController {
    ...
    //try! NSRegularExpression(pattern: "^(?=.*[A-Za-z])(?=.*\\d)(?=.*[$@$!%*#?&])[A-Za-z\\d$@$!%*#?&]{8,}$") //regex used to ensure password has 8 characters (at least 1 alphabet and 1 number and 1 special character)
    ...
    @IBAction func CreateAccountButtonPressed(_ sender: Any) {
        ...
        AgePickerView.isUserInteractionEnabled = false
        AddProfilePicButton.isUserInteractionEnabled = false
        
        LoadingIndicatorLabel.text = "Loading SMS Authentication Please Wait"
        LoadingIndicatorLabel.isHidden = false
        //prepare for and perform SMS verification call
        smsFormattedPhoneNumber="+1" + phoneNumber!
        PhoneAuthProvider.provider().verifyPhoneNumber(smsFormattedPhoneNumber!, uiDelegate: nil) { (verificationID, error) in
            if let e = error{
            ...
            if let validVerifID = verifID, let smsValue = sender.text{
                ...
                Auth.auth().signIn(with: credential) { (authResult, error) in
                    ...
                    Auth.auth().currentUser!.link(with: Ecredential){ authResult, error in
                        if let e = error{
                            //if there's an error linking account info, then we must delete the account so they can try again using same phoneNumber
                            Auth.auth().currentUser?.delete(completion: { (error) in
                                if let e = error{
                                    self.ErrorMessageLabel.text = "we linked your phone but not your email, call tech support"
                                    self.LoadingIndicatorLabel.isHidden = true
                                    return
                                }
                                self.ErrorMessageLabel.text = "couldn't link account Close app and try again"
                                self.LoadingIndicatorLabel.isHidden = true
                                return
                            })
                            self.LoadingIndicatorLabel.isHidden = true
                        ...
                        //need to check if username is taken or not at some point before this
                        //create document in "Active Users <username> " collection named with the <username>
                        self.db.collection(K.Fstore.usersCollection).document(self.username!).setData([K.Fstore.userUserName: self.username!,K.Fstore.userPhoneNumber: self.phoneNumber!, K.Fstore.userEmail: self.email!, K.Fstore.userAge: self.age!], merge: true)
